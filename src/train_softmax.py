...
def main(args):
    ...
    #nhập module (inception_resnet_v1)
    ...
    if not os.path.isdir(log_dir):  # tạo log nếu chưa tồn tại
        os.makedirs(log_dir)
    model_dir = os.path.join(os.path.expanduser(args.models_base_dir), subdir)
    if not os.path.isdir(model_dir):  # tạo model nếu chưa tồn tại
        os.makedirs(model_dir)

    stat_file_name = os.path.join(log_dir, 'stat.h5')

    # ghi arguments vào arguments.txt
    ...
    if args.filter_filename:
        ...
        # sử dụng 95% dữ liệu để training và 5% dữ liệu dùng để test
    ...
    with tf.Graph().as_default():
        ...
        # lấy path danh sách hình ảnh của nhãn
        image_list, label_list = facenet.get_image_paths_and_labels(train_set)
        assert len(image_list)>0, 'The training set should not be empty'
        
        val_image_list, val_label_list = facenet.get_image_paths_and_labels(val_set)

        # tạo một hàng đợi tạo ra danh sách list hình ảnh và nhãn
        ...
        #truyền dữ liệu khác nhau vào đồ thị khi training
        ...
        # Định mức cho các điều kiện tiên quyết
        eps = 1e-4
